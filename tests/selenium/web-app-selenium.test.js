...
class WebAppSeleniumTests {
    ...
    async testJobAlertSystem(driver, browser) {
        ...
        try {
            ...
            await driver.wait(until.stalenessOf(await driver.findElement(By.id('alert-form-modal'))), 5000);
    ...
    async testJobComparisonTool(driver, browser) {
        ...
        try {
            ...
            await driver.wait(until.stalenessOf(await driver.findElement(By.id('job-selection-modal'))), 5000);
    ...
    async testExportSharingFeatures(driver, browser) {
        ...
        try {
            ...
            // Test generate link
            const generateLinkButton = await driver.findElement(By.css('button[onclick*="generateShareLink"]'));
    ...
    async testWebSocketFunctionality(driver, browser) {
        ...
        try {
            ...
            assert(statusText.includes('Connected') || statusText.includes('Online'), 'WebSocket not connected');
    ...
    async runAllTests() {
        ...
        console.log(`📋 Testing ${TEST_CONFIG.browsers.length} browsers with ${TEST_CONFIG.browsers.join(', ')}`);
    ...
    async generateTestReport() {
        ...
        for (const [browser, results] of Object.entries(summary.byBrowser)) {
            console.log(`  ${browser}: ${results.passed}/${results.total} passed (${((results.passed/results.total)*100).toFixed(1)}%)`);
        }
