...
class ExportSharingSeleniumTests {
    ...
    async testQuickExportFunctionality(driver, browser) {
        ...
        try {
            ...
            // Test JSON export
            const jsonButton = await driver.findElement(By.css('button[onclick*="quickExport(\'json\')"]'));
            ...
            // Test Excel export
            const xlsxButton = await driver.findElement(By.css('button[onclick*="quickExport(\'xlsx\')"]'));
            ...
            for (const item of updatedQueueItems) {
                ...
                assert(['PENDING', 'PROCESSING', 'COMPLETED'].includes(status), `Invalid status: ${status}`);
            }
    ...
    async testSharingInterface(driver, browser) {
        ...
        try {
            ...
            const updateButton = await driver.findElement(By.css('button[onclick*="updateShareSettings"]'));
            ...
            await driver.wait(until.stalenessOf(await driver.findElement(By.id('share-link-modal'))), 5000);
    ...
    async testReportTemplates(driver, browser) {
        ...
        try {
            ...
            const createTemplateButton = await driver.findElement(By.css('button[onclick*="showCreateTemplateDialog"]'));
            ...
            const templateFieldCheckboxes = await driver.findElements(By.css('input[name="template-fields"]'));
            ...
            // Save template
            const saveTemplateButton = await driver.findElement(By.css('button[onclick*="saveTemplate"]'));
            await saveTemplateButton.click();
            
            // Wait for modal to close and template to appear
            await driver.wait(until.stalenessOf(await driver.findElement(By.id('create-template-modal'))), 5000);
            
            // Check templates grid
            await this.waitForElement(driver, By.id('templates-grid'));
            
            // Test import template button
            const importTemplateButton = await driver.findElement(By.css('button[onclick*="importTemplate"]'));
    ...
    async testJobCollections(driver, browser) {
        ...
        try {
            ...
            const createCollectionButton = await driver.findElement(By.css('button[onclick*="showCreateCollectionDialog"]'));
            ...
            // Save collection
            const saveCollectionButton = await driver.findElement(By.css('button[onclick*="saveCollection"]'));
            await saveCollectionButton.click();
            
            // Wait for modal to close
            await driver.wait(until.stalenessOf(await driver.findElement(By.id('create-collection-modal'))), 5000);
