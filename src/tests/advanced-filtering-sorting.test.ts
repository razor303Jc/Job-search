...
import { AdvancedFilteringSorting } from '../src/web/components/advanced-filtering-sorting';
...
describe('Advanced Filtering & Sorting Component', () => {
  ...
  describe('Filter Validation', () => {
    ...
    test('should require value for operators that need it', () => {
      component.addFilterGroup();
      const groupId = component.getFilterState().groups[0].id;
      component.addFilterCondition(groupId);
      ...
      const isValid2 = component.validateFilterCondition(groupId, conditionId, {
        field: 'title',
        operator: 'isEmpty',
        value: '', // Empty value should be valid for isEmpty
      });
      expect(isValid2).toBe(true);
    });
  });
